...
def get_model_complexity_info(model, input_res,
    ...
    if as_strings:
        return flops_to_string(flops_count, units='GMac'), params_to_string(params_count, units='G')
...
import argparse          
def get_args_parser():
    parser = argparse.ArgumentParser(description='Measure Cost')
    parser.add_argument('--model', type=str, default='vit', choices=MODELS)
    parser.add_argument('--dataset', default='T-IMNET', type=str)
    parser.add_argument('--type', default='flops', type=str, choices=['flops', 'latency'])
...
def main(args):
    ...
    if args.type == 'latency': 
        ...
        if args.model in ['effiv2', 'regnetX_400m', 'effiv2_m', 'regnetY_200m', 'regnetY_4G', 'regnetY_8G', 'regnetY_400m', 'regnetX_200m', 'res110', 'effib0']:
            # CNN flops
